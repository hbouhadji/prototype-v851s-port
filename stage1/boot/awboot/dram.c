...
static int ccu_set_pll_ddr_clk(int index, dram_para_t *para)
{
	...
	if (!wait_register(CCU_BASE + CCU_PLL_DDR_CTRL_REG, 0x10000000, 0x10000000, __LINE__)) return 0;
...
static unsigned int mctl_channel_init(unsigned int ch_index, dram_para_t *para)
{
	...
	if (dqs_gating_mode == 1) {
		...
		if (!wait_register(MCTL_PHY_BASE + MCTL_PHY_PGSR0, 0x1, 0x1, __LINE__)) return 0; // wait for IDONE
		udelay(10);
	...
	if (readl((SUNXI_R_CPUCFG_BASE + SUNXI_R_CPUCFG_SUP_STAN_FLAG)) & (1 << 16)) {
		...
		if (!wait_register(MCTL_PHY_BASE + MCTL_PHY_STATR, 0x7, 0x3, __LINE__)) return 0;
		...
		if (!wait_register(MCTL_PHY_BASE + MCTL_PHY_STATR, 0x7, 0x1, __LINE__)) return 0;
		...
		if (dqs_gating_mode == 1) {
			...
			if (!wait_register(MCTL_PHY_BASE + MCTL_PHY_PGSR0, 0x1, 0x1, __LINE__)) return 0;
		}
	}
	...
	if (!wait_register(MCTL_PHY_BASE + MCTL_PHY_STATR, 0x1, 0x1, __LINE__)) return 0;
